...
namespace ArtrointelPlugin.Utils
{
    ...
    {
        private IntPtr mHndIcon;
        private Bitmap mBmp;

        public LoadIconHelper(string filePath)
        {
            mHndIcon = GetJumboIcon(GetIconIndex(filePath));
            mBmp = Icon.FromHandle(mHndIcon).ToBitmap();
        }

        ~LoadIconHelper()
        {
            mBmp.Dispose();
            Shell32.DestroyIcon(mHndIcon);
        }

        public Image getJumboIcon()
        {
            return mBmp;
        }
